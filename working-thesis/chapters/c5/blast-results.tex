...
queries to\cbstart~tblastn~\cite{altschul1990a} \cbend on each \textit{Trichoderma}
assembly. The organisms and their associated NCBI accession IDs are as
follows: \textit{T. atroviride} --- GCF\_000171015.1,
\textit{F. graminarium} --- GCF\_000240135.3, \textit{S. cerevisiae} --- GCF\_000146045.2. These selected assemblies range in evolutionary distance from the \textit{Trichoderma} genomes being analyzed, with \textit{T. atroviride} being the closest relative, followed by \textit{F. graminarium}, and finally \textit{S. cerevisiae} as the most distantly related organism.
Results from the tblastn runs are presented in
Figure~\ref{fig:blast-total-counts} and Table~\ref{table:tblastn-prots}. 
Initial tblastn results appear
promising for both the \textit{T. atroviride} and \textit{F. graminarium}
...
a minimum of 75\% in the case of \textit{F. graminarium}. In the case of
