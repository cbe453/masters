...
promising performance for Braker2 and GeneMark as the RefSeq annotations
...
an indication of the presence of Pfam matches. In the IGV screenshots, Pfam matches are offset
...
RefSeq only reports one gene with multiple Pfam hits. Even more interesting are
...
is disagreement in several forms.\cbstart~We also note that 
the lack of a Pfam match does not necessarily indicate an incorrect gene prediction, as not all proteins have known domains in the Pfam database. Further investigation of genes without Pfam is still warranted to determine their validity.\cbend
