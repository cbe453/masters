...
distribution function (CDF) for the lengths of CDS sequences for each gene
...
GeneMark. In the case of Tsth20, the
...
gene lengths. In \textit{T. harzianum}, Braker2 deviates from GeneMark
and RefSeq, predicting more genes of short length, while GeneMark and
RefSeq appear to be in near-complete agreement except in the case of
very short genes. Finally, in \textit{T. virens}, we see that RefSeq predicts a larger fraction of shorter genes than Braker2 and GeneMark. Interestingly, Braker2 does not predict the shortest genes in\textit{T. virens}, but why this is the case is unclear.
From these plots, we can say that visually, gene
...
\begin{figure}
  ...
  \caption[CDF plots for DC1 and Tsth20]{Plots of the cumulative distribution
    function for CDS lengths produced by each gene finding tool
...
\cbstart To confirm that the CDF curves differ, two-sided two-sample
Kolmogorov-Smirnov~\cite{2008}\cbend~tests were performed using the
...
that the difference between the distributions of frequencies of gene lengths predicted by
 any two tools can be explained by random chance. We set a \textit{p-}value threshold of 0.005 for statistical significance. Results are presented in Table~\ref{table:ks-2s}. In the cases of DC1 and Tsth20, we see that in
...
other. It is also notable that RefSeq and GeneMark do not predict similar
gene lengths in \textit{T. harzianum}, \textit{T. harzianum}, and \textit{T. virens}, although the \textit{p-}value of their comparison in \textit{T. reesei} is close to the significance threshold.
...
It can be stated that there are differences between some of the distributions of gene lengths predicted by these tools, particularly in
...
trained. Conversely, we observe that when Braker2 is trained
