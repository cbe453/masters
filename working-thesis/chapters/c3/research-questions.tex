...
\cbcolor{orange}
\cbstart
Gene finding tools vary widely in their implementation, ease of use, and computational requirements. Before delving into the performance of these tools, it is important to profile their characteristics. We ask:
\textbf{How are these gene
  finding tools implemented? Are the tools easy to install and use? What is the processing time of different gene finding tools in the
...
Secondary metabolites produced by \textit{Trichoderma} species are of great interest due to their potential applications in agriculture and medicine. Identifying biosynthetic gene clusters responsible for the production of these secondary metabolites is crucial for understanding their biosynthesis and potential utility.\cbstart We ask: \textbf{using antiSMASH, can we identify secondary metabolite biosynthetic gene cluster candidates in Tsth20 from Braker2 and GeneMark gene predictions?}\cbend
