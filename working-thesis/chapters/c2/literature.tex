...
%genome consists of tightly bundled sequences of DNA, which are
...
This chapter provides a brief background on \textit{Trichoderma} and
gene prediction tools, giving context for this
research. Section~\ref{lit:evolution} discusses the evolutionary
mechanisms that may have led to the increased production of secondary
metabolites in \textit{Trichoderma} species, such as horizontal gene
transfer, gene duplication, and transposable
elements. Section~\ref{lit:novel-genomes} discusses the
novel \textit{Trichoderma} genomes that were sequenced and assembled
as part of this work, and how they relate to the research
questions. Section~\ref{lit:similarity-searches} discusses the
structure of eukaryotic genes as well as a background of gene finding
tools and methods, while Section~\ref{lit:secondary-metabolites}
provides an overview of secondary metabolites and their importance
in \textit{Trichoderma} species. Finally, Section~\ref{lit:comp-motiv}
provides an overview of comparative studies of gene prediction tools
in \textit{Trichoderma} species, and how this work aims to fill the
gap in literature.

\section{\textit{Trichoderma} Species and their Features}\label{lit:features}

\textit{Trichoderma} species are a group of fungi in the \textit{Hypocreaceae}
family and are found in a wide variety of soils acting as non-toxic, avirulent, opportunistic plant symbionts, and also known for
their use in biomanufacturing of cellulases and hemicellulases~\cite{woo2023a, kubicek2019a}.  Many of these features
of \textit{Trichoderma} can be attributed to the large number of
secondary metabolites produced by these fungi, which 
...
such production~\cite{Mukherjee2012}. Lastly,
...
Interestingly, both HGT and TEs tend to present themselves in regions
...
in \textit{Trichoderma} due to repeat-induced point (RIP) mutations,
...
with other organisms. While difficult to prove, studies have shown
...
content~\cite{tsth20-hydrocarbon}. DC1\footnote{DC1 was isolated by members of the Kaminskyj lab in the Biology Department at the University of Saskatchewan. There is no published material on this organism at this time.}, while not yet classified, is believed to confer salt and
...
work in these genomes. While this thesis does not explicitly
...
of \textit{Trichoderma}. The assembly of these genomes comes as a
...
The evolutionary processes in \textit{Trichoderma} species is an important aspect of this work. DC1 and Tsth20 have not yet been classified into a specific \textit{Trichoderma} species, although Tsth20 has been tentatively declared a strain of \textit{T. harzianum}~\cite{tsth20-hydrocarbon}, and further understanding of their evolutionary relationships to other \textit{Trichoderma} species may help explain how DC1 and Tsth20 originated. 
...
organism and may be necessary for survival in some situations~\cite{Craney2013, Mukherjee2012}. These other cellular products are known
 ...
 \cbstart~as they are generally not expressed under standard laboratory conditions~\cite{Mukherjee2012}. 
 These clusters contain several genes that work together to produce a specific secondary metabolite, with each gene encoding a module of the NRPS or PKS enzyme complex. Synthesis of the secondary metabolites do not involve the ribosome, which is the typical cellular machinery used to produce proteins, hence the name non-ribosomal peptide synthetases~\cite{komaki2020}.
  Despite difficulties in studying these products, many secondary metabolites have been identified and studied for their potential applications in medicine and agriculture~\cite{Craney2013, Mukherjee2012}.
 \cbend
...
With the biological context laid out, we can now turn our attention to the computational aspect of this work. Many \textit{Trichoderma} species have been assembled and annotated in the past, mostly for the purpose of understanding the mechanisms behind secondary metabolite production~\cite{Mukherjee2012}. Gene predictions in this context are typically compared against sequences from other organisms to derive functional information, and to validate the predicted genes~\cite{loftus2003a}.\cbstart~In contrast, although some studies exist~\cite{ter-hovhannisyan2008}\cite{keilwagen2018}, few studies have been conducted comparing gene prediction tools in fungal genomes, and after an exhaustive search, no studies were found comparing gene finding tools specifically in \textit{Trichoderma} species\cbend. This is surprising, given the large number of gene prediction tools available, and the fact that many of them are used in the field of fungal genomics~\cite{ejigu2020a}. Comparisons of gene prediction tools are common when new tools are released, but these comparisons are often limited to a few tools, and tend to be focused on achieving higher counts of interesting genes, rather than comparing the results in a biological context~\cite{min2017}. More importantly, these comparisons usually involve model organisms and typically do not include lesser studied organisms like \textit{Trichoderma}. Complicating the matter further, the quality of genome sequences available may vary, making evaluation of different gene prediction tools difficult when working with multiple genomes and lesser-studied fungal species with low quality assemblies. Given the gap in literature regarding comparative analysis of gene prediction tools in \textit{Trichoderma} species, and the availability of the novel DC1 and Tsth20 genomes, this work aims to fill that gap by comparing the outputs of several gene prediction tools on these two genomes along with three other frequently studied \textit{Trichoderma} species. 
